...
test.group('Auth', (group) => {
  ...
  test('register a new user', async ({ client, assert }) => {
    ...
    assert.isFalse(user?.canManageSchool(school.id), 'Should not manage school')
  })
  ...
  test('logout authenticated user', async ({ client }) => {
    ...
    const response = await client.delete('/logout').bearerToken(token.value!.release())
  ...
  test('cannot signup with already used invitation code', async ({ client }) => {
    const invitationCode = crypto.randomUUID()
    await User.create({
  ...
  test('account lockout after failed login attempts', async ({ client, assert }) => {
    const user = await User.create({
  ...
  test('logout from all devices', async ({ client, assert }) => {
    ...
    const response = await client.delete('/logout-all').bearerToken(token1.value!.release())
    ...
    const meResponse1 = await client.get('/me').bearerToken(token1.value!.release())
    const meResponse2 = await client.get('/me').bearerToken(token2.value!.release())
